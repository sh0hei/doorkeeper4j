...
public class DoorKeeperClientTestUtil {
    ...
    static void configureMock(final DoorKeeperExecutor executor, final int statusCode) throws IOException {
        configureMock(executor, statusCode, null);
    }

    @SuppressWarnings({ "resource", "unused" })
    static void configureMock(
            final DoorKeeperExecutor executor, final int statusCode, String contentFileName) throws IOException {
        final InputStream content;
        if (StringUtils.isEmpty(contentFileName)) {
            content = null;
        } else {
            content = DoorKeeperClientTestUtil.class.getResourceAsStream(contentFileName);
        }
